...
class MNASNet(torch.nn.Module):
    ...
    def __init__(self, alpha, num_classes=1000, dropout=0.2, **kwargs):
        super(MNASNet, self).__init__()
        assert alpha > 0.0
        self.alpha = alpha
        self.num_classes = num_classes
        depths = _get_depths(alpha)
        print(f'depths = {str(depths)}')
        #layers = [
        ...
        self.layers = nn.Sequential(*layers)
        # MNASNet blocks: stacks of inverted residuals.
        self.mnasnet_block_1 = _stack(depths[1], depths[2], 3, 2, 3, 3, _BN_MOMENTUM)
        self.mnasnet_block_2 = _stack(depths[2], depths[3], 5, 2, 3, 3, _BN_MOMENTUM)
        self.mnasnet_block_3 = _stack(depths[3], depths[4], 5, 2, 6, 3, _BN_MOMENTUM)
        self.mnasnet_block_4 = _stack(depths[4], depths[5], 3, 1, 6, 2, _BN_MOMENTUM)
        self.mnasnet_block_5 = _stack(depths[5], depths[6], 5, 2, 6, 4, _BN_MOMENTUM)
        self.mnasnet_block_6 = _stack(depths[6], depths[7], 3, 1, 6, 1, _BN_MOMENTUM)
        ## Final mapping to classifier input.
        #nn.Conv2d(depths[7], 1280, 1, padding=0, stride=1, bias=False),
        #nn.BatchNorm2d(1280, momentum=_BN_MOMENTUM),
        #nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        #x = x.mean([2, 3])
        x_1 = self.mnasnet_block_1(x_0) 
        x_2 = self.mnasnet_block_2(x_1)
        x_3 = self.mnasnet_block_3(x_2)
        x_4 = self.mnasnet_block_4(x_3)
