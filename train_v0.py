        ...
        dataset_train = CVIDataset(args.dataset_root, set_name='train', annotation_name='thermal_annotations.json',
                                    transform=transforms.Compose(
                                        [
                                            Normalizer(), 
                                            Augmenter(), 
                                            Resizer(min_side=int(_h), max_side=int(_w), base_size=16, logger=net_logger)]))
        dataset_valid = CVIDataset(args.dataset_root, set_name='valid', annotation_name='thermal_annotations.json',
                                  transform=transforms.Compose(
                                      [
                                          Normalizer(), 
                                          Resizer(min_side=int(_h), max_side=int(_w), base_size=16)]))
    elif args.dataset == '3s-pocket-thermal-face':
        if args.dataset_root is None:
            raise ValueError('Must provide --dataset_root when training on thermal,')
        _h, _w = 288, 384
        dataset_train = CVIDataset(args.dataset_root, set_name='train', annotation_name='annotations.json',
                                   transform=transforms.Compose(
                                        [
                                            Normalizer(), 
                                            Augmenter(), 
                                            Resizer(height=int(_h), width=int(_w), resize_mode=1, logger=net_logger)]))
        #dataset_valid = CVIDataset(args.dataset_root, set_name='valid', annotation_name='annotations.json',
        dataset_valid = CVIDataset(args.dataset_root, set_name='train', annotation_name='annotations.json',
                                   transform=transforms.Compose(
                                      [
                                          Normalizer(), 
                                          Resizer(height=int(_h), width=int(_w), resize_mode=1)]))
    else:
