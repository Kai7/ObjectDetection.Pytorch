"""
  Modified by KaiSeven Yang
    source from: torchvision.models.shufflenetv2.py
"""
...
__all__ = ['ShuffleNetV2', 'ShuffleNetV2Features']
...
default_cfgs = {
    'shufflenetv2_x0.5': _cfg(url='https://download.pytorch.org/models/shufflenetv2_x0.5-f707e7126e.pth'),
    'shufflenetv2_x1.0': _cfg(url='https://download.pytorch.org/models/shufflenetv2_x1-5666bf0f80.pth'),
    'shufflenetv2_x1.5': _cfg(),
    'shufflenetv2_x2.0': _cfg(),
}
...
class ShuffleNetV2Features(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, num_classes=1000, inverted_residual=InvertedResidual):
        ...
        output_channels = self._stage_out_channels[-1]
        self.feature_maps_channels = stages_out_channels[1:-1]
        # self.conv5 = nn.Sequential(
        #     nn.Conv2d(input_channels, output_channels, 1, 1, 0, bias=False),
        #     nn.BatchNorm2d(output_channels),
        #     nn.ReLU(inplace=True),
...
def shufflenet_v2_x0_5(pretrained=False, num_classes=1000, in_chans=3, progress=True, **kwargs):
    ...
    """
    default_cfg = default_cfgs['shufflenetv2_x0.5']
    if kwargs.pop('features_only', False):
        model = ShuffleNetV2Features([4, 8, 4], [24, 48, 96, 192, 1024], **kwargs)
    else:
        model = ShuffleNetV2([4, 8, 4], [24, 48, 96, 192, 1024], num_classes=num_classes, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans, filter_fn=_filter_pretrained)
    return model
...
def shufflenet_v2_x1_0(pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    default_cfg = default_cfgs['shufflenetv2_x1.0']
    if kwargs.pop('features_only', False):
        model = ShuffleNetV2Features([4, 8, 4], [24, 116, 232, 464, 1024], **kwargs)
    else:
        model = ShuffleNetV2([4, 8, 4], [24, 116, 232, 464, 1024], num_classes=num_classes, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans, filter_fn=_filter_pretrained)
    return model
...
def shufflenet_v2_x1_5(pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    default_cfg = default_cfgs['shufflenetv2_x1.5']
    if kwargs.pop('features_only', False):
        model = ShuffleNetV2Features([4, 8, 4], [24, 176, 352, 704, 1024], **kwargs)
    else:
        model = ShuffleNetV2([4, 8, 4], [24, 176, 352, 704, 1024], num_classes=num_classes, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans, filter_fn=_filter_pretrained)
    return model
...
def shufflenet_v2_x2_0(pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    default_cfg = default_cfgs['shufflenetv2_x2.0']
    if kwargs.pop('features_only', False):
        model = ShuffleNetV2Features([4, 8, 4], [24, 244, 488, 976, 1024], **kwargs)
    else:
        model = ShuffleNetV2([4, 8, 4], [24, 244, 488, 976, 1024], num_classes=num_classes, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans, filter_fn=_filter_pretrained)
    return model
